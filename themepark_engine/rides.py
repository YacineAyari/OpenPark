...
class Ride:
    def __init__(self, defn: RideDef, x: int, y: int):
        ...
        self.ride_duration = 3.0  # Default ride duration in seconds
    ...
    def board_visitor(self, visitor: 'Guest') -> bool:
        ...
        if self.can_board():
            ...
            DebugConfig.log('rides', f"Visitor {visitor.id} boarded ride {self.defn.name}. Total visitors: {len(self.current_visitors)}")
    ...
    def _handle_breakdown(self):
        """Handle ride breakdown - evacuate visitors and clear queue"""
        # Evacuate current visitors
        for visitor in self.current_visitors:
            ...
            DebugConfig.log('rides', f"Evacuating visitor {visitor.id} due to breakdown")
        ...
        DebugConfig.log('rides', f"Ride {self.defn.name} evacuated all visitors due to breakdown")
